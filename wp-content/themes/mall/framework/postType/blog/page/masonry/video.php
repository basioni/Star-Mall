	...
	<?php
		...
		if (!post_password_required()) {
			if ($cmsmasters_post_video_type == 'selfhosted' && !empty($cmsmasters_post_video_links) && sizeof($cmsmasters_post_video_links) > 0) {
				...
				echo '<div class="cmsmasters_video_wrap">' . 
					do_shortcode($wp_embed->run_shortcode('[embed width="' . $video_size['blog-masonry-thumb']['width'] . '" height="' . $video_size['blog-masonry-thumb']['height'] . '"]' . $cmsmasters_post_video_link . '[/embed]')) . 
				'</div>';
